...
export function createInitialState(criteria: ReviewCriterion[]): ReviewWorkspaceState {
  const evaluations: Record<number, CriterionEvaluation> = {};
  for (const criterion of criteria) {
    evaluations[criterion.id] = { criterionId: criterion.id, rating: null, comment: "" };
  }
  return {
    evaluations,
    generalComments: {
...
export type ReviewWorkspaceAction =
  ...
  | { type: "SUBMIT_START" }
  | { type: "SUBMIT_SUCCESS"; submissionResult: SubmissionResult }
  | { type: "SUBMIT_ERROR" };
  ...
  switch (action.type) {
    case "SET_CRITERION_RATING":
      return {
        ...
        evaluations: {
          ...state.evaluations,
          [action.id]: { ...state.evaluations[action.id], rating: action.rating },
        },
      };
    ...
    case "SET_CRITERION_COMMENT":
      return {
        ...
        evaluations: {
          ...state.evaluations,
          [action.id]: { ...state.evaluations[action.id], comment: action.comment },
        },
      };
    ...
    case "SET_GENERAL_COMMENT":
      return {
        ...state,
        generalComments: { ...state.generalComments, [action.field]: action.value },
      };
...
export function selectCompletedCount(state: ReviewWorkspaceState): number {
  return Object.values(state.evaluations).filter((e) => e.rating !== null).length;
}
