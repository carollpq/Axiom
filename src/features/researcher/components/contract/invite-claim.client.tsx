...
export function InviteClaimClient({
  ...
  const walletMatches =
    connectedWallet.toLowerCase() === contributorWallet.toLowerCase();
  ...
  if (signed) {
    return (
      <div className="text-center py-8">
        <div className="text-[#8fbc8f] text-lg mb-2">
          {'\u2713'} Contract Signed
        </div>
        <div className="text-[12px] text-[#6a6050]">
          Redirecting to the contract builder…
        </div>
      </div>
    );
  }
  ...
  if (!walletMatches) {
    return (
      ...
      >
        <div className="text-[12px] text-[#d4645a] mb-1">
    ...
    <div>
      ...
      <button
        ...
        style={{
          ...
          border:
            '1px solid ' +
            (signing ? 'rgba(120,110,95,0.15)' : 'rgba(180,160,120,0.4)'),
