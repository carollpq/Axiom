...
export function AuthorSearch({ onSelect, onCancel }: AuthorSearchProps) {
  ...
  function handleChange(value: string) {
    ...
    timerRef.current = setTimeout(async () => {
      setLoading(true);
      try {
        const data = await fetchApi<UserSearchResult[]>(`/api/users/search?q=${encodeURIComponent(value.trim())}`);
        setResults(data ?? []);
        setOpen(true);
      } catch {
        setResults([]);
  ...
  return (
    ...
    >
      <div className="flex-1 relative">
        ...
        />
        {loading && (
          <span className="absolute right-3 top-2.5 text-[10px] text-[#6a6050]">searching…</span>
        )}
        {open && results.length > 0 && (
          <div
            className="absolute left-0 right-0 mt-1 rounded overflow-hidden z-10"
            style={{ background: "rgba(30,28,24,0.95)", border: "1px solid rgba(120,110,95,0.25)" }}
          >
            {results.map(r => (
              <button
                key={r.id}
              ...
              >
                <div className="text-[12px] text-[#d4ccc0] font-serif">
                  ...
                  {r.orcidId && <span className="text-[10px] text-[#5a7a9a] ml-2">{r.orcidId}</span>}
                </div>
                <div className="text-[10px] text-[#6a6050] font-mono mt-0.5">{r.walletAddress}</div>
              </button>
            ))}
          </div>
        )}
