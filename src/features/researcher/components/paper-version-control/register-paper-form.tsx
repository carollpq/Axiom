...
export function RegisterPaperForm({
  ...
  if (upload.registered) {
    return (
      ...
      >
        <div className="text-[14px] text-[#8fbc8f] mb-2">
          Paper registered successfully
        </div>
        ...
        {upload.fileHash && (
          <div className="text-[11px] text-[#8a8070] mt-1">
            A unique digital fingerprint has been recorded and timestamped on
            the blockchain for this file.
          </div>
        )}
        <button
          onClick={onReset}
  ...
  return (
    <>
      ...
      <div className="mb-[18px]">
        ...
        />
        {uploadErrors.title && (
          <div className="text-[10px] text-[#d4645a] mt-1">
            {uploadErrors.title}
          </div>
        )}
      </div>
        ...
        {upload.keywords.length < PAPER_LIMITS.keywords.max && (
          <div className="flex gap-2">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && upload.keywordInput.trim())
                  upload.addKeyword();
              }}
              disabled={isFormBusy}
